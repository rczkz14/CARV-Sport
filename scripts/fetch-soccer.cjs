...
async function fetchSoccerMatches() {
  ...
  try {
    ...
    // Date filter: today to 5 days ahead
      ...
      for (const match of eplUpcoming.events.slice(0, 10)) { // Limit to 10
        if (match.idEvent) {
          ...
          if (liveData?.events?.[0]) {
            ...
            // Filter by date: only include matches from today up to 5 days ahead
            if (evt.dateEvent) {
              const matchDate = new Date(`${evt.dateEvent}T${evt.strTime || '15:00:00'}Z`);
              if (matchDate >= today && matchDate <= fiveDaysAhead) {
                matches.push({
