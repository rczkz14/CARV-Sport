...
export async function POST(request: Request) {
  ...
  // Debug: log env and input
  console.log('[DEBUG] SUPABASE_URL:', process.env.NEXT_PUBLIC_SUPABASE_URL);
  console.log('[DEBUG] SUPABASE_ANON_KEY:', process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY?.slice(0,8));
  ...
  const { data: existing, error: existingError } = await supabase
    ...
    .eq("buyer", buyer);
  if (existingError) {
    console.error('[DEBUG] Supabase select error:', existingError);
  ...
  if (existing && existing.length > 0) {
    ...
    // Use provided prediction or fallback (generate in-memory, do NOT read/write any file)
    ...
    if (!finalPrediction) {
      ...
      if (match) {
        // Generate prediction in-memory only
        try {
          // Generate prediction with Predicted Score, Total Score, Winner
          const homeScore = Math.floor(Math.random() * 30) + 95; // 95-125 for NBA
          const awayScore = Math.floor(Math.random() * 30) + 95; // 95-125 for NBA
          ...
          finalPrediction = "Prediction generation failed. Please try again later.";
        }
      } else {
  ...
  // Debug: log record to insert
  console.log('[DEBUG] Inserting record:', rec);
